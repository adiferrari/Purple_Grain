...
typedef struct pd_granular_synth_tilde
{
    ...
    float soundfile_length_ms;                          ///< lenght of the soundfile in milliseconds
    t_word *envelopeTable;                              ///< array containing the envelope
...
t_int *pd_granular_synth_tilde_perform(t_int *w)
{
    ...
    c_granular_synth_properties_update(x->synth, x->grain_size, x->start_pos, x->time_stretch_factor, x->midi_velo, x->midi_pitch, x->attack, x->decay, x->sustain, x->release, x->gauss_q_factor); ///< passes all (slider) changes to synth
